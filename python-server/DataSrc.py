...
class HisQuotes(Base):
        ...
        try:
            if ts_code is None:
                ...
                for i, r in df.iterrows():
                    time.sleep(1)
                    print('\r' + str(i + 1) + '/' + str(len(df)) + '（' + r['ts_code'] + '）', end='\n', flush=False)
                    data = self.pro.fut_daily(ts_code=r['ts_code'])

                    ins_type = 'replace' if i == 0 else 'append'
                    data.to_sql(r['fut_code'].upper(), self.conn, index=False, if_exists=ins_type)
            else:
    ...
    def getData(self, ts_code, start, end, short=5, long=10):
        ...
        with TradeCal(self.exchange) as tc:
            ...
            print("数量", len(tradecal))

        # 库中不存在完整数据
        if len(data) != len(tradecal):
            print("本地数据不完整", len(data))
        ...
        data['MAS'] = round(data.close.rolling(short, min_periods=1).mean(), 2)
        data['MAL'] = round(data.close.rolling(long, min_periods=1).mean(), 2)
