...
function Send(){
    ...
    let post_data = {'code': '8105', 'fut': fut.value, 'start': start.value, 'end': end.value,
                     'long': parseInt(long.value), 'short': parseInt(short.value), 'cash': parseInt(cash.value)};
...
function doReceive(buffer) {
    ...
    console.log(buffer)
    let msg = JSON.parse(buffer).info
...
$(document).ready(function() {   //在HTML文档加载完成后执行
    ...
    $("#DownloadData").click(function(){
        ...
        let post_data = {'code': '8008', 'fut': fut.value, 'start': start.value, 'end': end.value, 'shortT': short.value, 'longT': long.value};
            ...
            success: function (data, status) {
                ...
                if (status == 'success') {
                    ...
                    for (let i = 0; i < json.length; i++) {
                        //处理数据放到数组里
                        const arr = new Array(json[i].trade_date,
                            [json[i].open, json[i].close, json[i].low, json[i].high], json[i].ts_code, [json[i].MAS, json[i].MAL]);
                        dataArr.push(arr)
                    }
                    console.log(dataArr)
                    document.getElementById("ShowInfo").innerHTML = "数据下载完成</br>";
                    goChart(document.getElementById("chart"), dataArr);
                } else {
                    document.getElementById("ShowInfo").innerHTML = "数据下载失败</br>";
                }
            }
        })
        Connect();
    ...
    function goChart(cBox, dataArr){
        ...
        function drawBarAnimate(mouseMove){
            ...
            // 绘制折线图
            //（1）五日均线
            ctx.strokeStyle = "rgb(85,137,243)";  //"#49FE79";
            // 连线
            ctx.beginPath();
            for(var i = 0; i < tobalBars; i++){
        ...
        function drawDragBar(){
            drawRect(originX, originY+cSpace+5, cWidth, cMargin-20, false, "#FFFFFF");
            drawRect(originX, originY+cSpace+5, dragBarX-originX, cMargin-20, false, "#afb3b8");
            drawRect(dragBarX, originY+cSpace+5, dragBarWidth, cMargin-20, false, "#727070",false,true);
        }
        // 监听拖拽
